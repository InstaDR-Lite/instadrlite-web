...
export interface MediaDanceClientEvents {
  ...
  'room-allocated': (payload: { roomId: string }) => void;
  'bitrate-adapted': (tier: string) => void; // 💡 Added clean metric hook for client UI awareness
}
...
export class MediaDanceClient extends EventEmitter {
  ...
  private config: MediaDanceConfig;
  private bitrateAdapter: BitrateAdapter | null = null; // 💡 Background adaptive tracking reference
   ...
   */
  private orchestrateEvents(): void {
    ...
    this.signaling.on('peer-notification', async (data: { senderSocketID: string; sdp?: RTCSessionDescriptionInit; candidate?: RTCIceCandidateInit }) => {
      ...
      try {
        if (data.sdp) {
          ...
          if (localAnswer) {
            ...
            if (pc) this.initializeBitrateTracking(pc);
          } else {
   ...
   */
  public async createCallOffer(targetSocketId: string): Promise<void> {
    try {
   ...
   */
  private initializeBitrateTracking(pc: RTCPeerConnection): void {
    if (this.bitrateAdapter) this.bitrateAdapter.stop();
  ...
  public muteAudio(isMuted: boolean): void { this.media.toggleTrack('audio', !isMuted); }
  public muteVideo(isMuted: boolean): void { this.media.toggleTrack('video', !isMuted); }
