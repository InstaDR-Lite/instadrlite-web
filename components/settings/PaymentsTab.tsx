...
function PaymentsInner() {
  ...
  const handleConnectPayPal = async () => {
    setConnecting(true);
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/paypal/onboard`, {
      credentials: 'include'
    });
    const { url } = await res.json();
    setPaypalUrl(url);
    setConnecting(false);
    window.open(url, 'PPFrame', 'width=700,height=700,scrollbars=yes');
  };
        ...
        <div className="border border-[rgba(0,80,40,0.18)] p-4 flex flex-col gap-4">
         ...
         {/* PayPal option */}
          <div className={`p-3 border transition-all ${
            ...
            <p className="text-[11px] text-[#7A9A7A] font-mono mb-3">
              Route session fees directly to your PayPal business account.
            </p>
            ...
            {provider?.paypal_merchant_id ? (
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-[11px] font-mono text-[#007A40]">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#007A40]" />
                  Connected: {provider.paypal_merchant_id}
                </div>
                <button className="text-[10px] tracking-widest uppercase text-[#CC2200] hover:underline font-mono">
                  disconnect
                </button>
              </div>
            ) : (
                ...
                onClick={(e) => {
                  ...
                  window.open(paypalUrl, 'PPFrame', 'width=700,height=700,scrollbars=yes');
                }}
                className="w-full py-2 border border-[rgba(0,80,40,0.18)] text-[10px] tracking-widest uppercase text-[#7A9A7A] hover:border-[#007A40] hover:text-[#007A40] transition-all block text-center cursor-pointer"
              >
                {connecting ? '// loading...' : '⚡ connect paypal account'}
              </a>
            )}
          </div>
