...
const INSURANCE_NETWORKS = [
  'Anthem', 'Aetna', 'Blue Cross Blue Shield', 'Cigna',
  'United Healthcare', 'Humana', 'Kaiser', 'Molina',
...
export default function RoomTab() {
  ...
  const handleInsuranceDone = async () => {
    setShowInsuranceModal(false);
    // Save just the insurance_networks immediately
    await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/auth/profile/public`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({ insurance_networks: profile.insurance_networks })
    });
  };
  const toggleNetwork = (network: string) => {
    const current = profile.insurance_networks ?? [];
    const updated = current.includes(network)
      ? current.filter((n: string) => n !== network)
      : [...current, network];
    setProfile((p: any) => ({ ...p, insurance_networks: updated }));
  };
    ...
    <>
      {showInsuranceModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-[#1A2E1A]/40 animate-fadeIn">
          <div className="bg-[#F5F0E8] border border-[rgba(0,80,40,0.18)] p-6 w-[360px] animate-slideUp">
            <div className="font-mono text-[10px] tracking-widest text-[#7A9A7A] uppercase mb-4">
              // accepted insurance networks
            </div>
            <div className="flex flex-col gap-3">
              {INSURANCE_NETWORKS.map(network => (
                <label key={network} className="flex items-center gap-3 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={(profile.insurance_networks ?? []).includes(network)}
                    onChange={() => toggleNetwork(network)}
                    className="accent-[#007A40]"
                  />
                  <span className="font-mono text-[13px] text-[#1A2E1A]">{network}</span>
                </label>
              ))}
            </div>
            <button
              onClick={() => handleInsuranceDone()}
              className="mt-6 w-full py-2.5 border border-[#007A40] text-[10px] tracking-widest uppercase text-[#007A40] hover:bg-[#007A40] hover:text-[#F5F0E8] transition-all font-mono"
            >
              Done
            </button>
          </div>
            ...
            <>
              ...
              <div className="border border-[rgba(0,80,40,0.18)] p-4 flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <label key="accepts_insurance" className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={profile.accepts_insurance ?? false}
                      onChange={e => {
                        setProfile((p: any) => ({ ...p, accepts_insurance: e.target.checked }));
                        if (e.target.checked) setShowInsuranceModal(true);
                      }}
                      className="accent-[#007A40]"
                    />
                    <span className="text-sm font-mono text-[#3D5C3D]">Insurance</span>
                    {profile.accepts_insurance && (profile.insurance_networks?.length > 0) && (
                      <button
                        onClick={() => setShowInsuranceModal(true)}
                        className="text-[10px] font-mono text-[#007A40] underline"
                      >
                        ({profile.insurance_networks.length} networks)
                      </button>
                    )}
                  </label>
                  {[
